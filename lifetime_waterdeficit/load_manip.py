...
def load_isimip(
    ...
    if flag_run: 
        ...
        # differentiation between modes through if statements - can be cleaned up
        for mode in modes: 
            print(mode)
            ...
            for extreme in extremes:
                ...
                # do reading and all for exposure
                if not  (mode == 'duration' or mode=='frequency' or mode == 'events'): 
                    # loop over models
                    for model in models: 
                        ...
                        file_names = glob.glob('./data/isimip/'+extreme+'/'+model.lower()+'/'+model.lower()+'*ssp*'+extreme+'*annual*landarea*2100*')

                        if extreme == 'waterdeficit' or extreme =='withdrawal': 
                            file_names = glob.glob('./data/isimip/'+extreme+'/'+model.lower()+'/'+model.lower()+'*ssp*'+extreme+'*annual*2100*')

                        if phase == '2b': 
                            file_names = glob.glob('./data/isimip/2b/'+extreme+'/'+model.lower()+'/'+model.lower()+'*rcp*'+extreme+'_*annual*landarea*2099*')

                            if extreme == 'waterdeficit' or extreme =='withdrawal' or extreme =='waterdeficitintensity': 
                                file_names = glob.glob('./data/isimip/2b/'+extreme+'/'+model.lower()+'/'+model.lower()+'*rcp*'+extreme+'*annual*2099*')
                        ...
                        for file_name in file_names: 
                            ...
                            d_isimip_meta[i] = {
                                ...
                                'rcp': file_name.split('_')[rcp_split],                         
                                'extreme': file_name.split('_')[extreme_split], 
                                'mode': mode,
                            }

                            if phase == '2b':
                                isimip_dir = './data/isimip/2b/'
                                picontrol_name = '_piControl_'
                                tas_filename = '*.fldmean.yearmean.txt'

                            elif phase == '3b':

                                isimip_dir = './data/isimip/'
                                picontrol_name = '_picontrol_'
                                tas_filename = '*tas_global_annual*.txt'

                            # load associated historical variable
                            if extreme == 'waterdeficit' or extreme =='withdrawal' or extreme =='waterdeficitintensity': 
                                file_name_his = glob.glob(isimip_dir+extreme+'/'+model.lower()+'/'+model.lower()+'*'+d_isimip_meta[i]['gcm']+'*_historical_*'+extreme+'_'+'*annual*')[0]
                            else: 

                                file_name_his = glob.glob(isimip_dir+extreme+'/'+model.lower()+'/'+model.lower()+'*'+d_isimip_meta[i]['gcm']+'*_historical_*'+extreme+'_'+'*annual*landarea*')[0]
                            ...
                            if d_isimip_meta[i]['gcm'] == 'hadgem2-es': # .upper() method doesn't work for HadGEM2-ES on linux server (only Windows works here)
                                file_names_gmt = glob.glob('./data/isimip/DerivedInputData/globalmeans/tas/HadGEM2-ES/'+tas_filename) # ignore running mean files
                            elif d_isimip_meta[i]['gcm'] == 'canesm5': # .upper() method doesn't work for HadGEM2-ES on linux server (only Windows works here)
                                file_names_gmt = glob.glob('./data/isimip/DerivedInputData/globalmeans/tas/CanESM5/'+tas_filename) # ignore running mean files
                            elif d_isimip_meta[i]['gcm'] == 'ec-earth3': # .upper() method doesn't work for HadGEM2-ES on linux server (only Windows works here)
                                file_names_gmt = glob.glob('./data/isimip/DerivedInputData/globalmeans/tas/EC-Earth3/'+tas_filename) # ignore running mean files
                            else:
                                file_names_gmt = glob.glob('./data/isimip/DerivedInputData/globalmeans/tas/'+d_isimip_meta[i]['gcm'].upper()+'/'+tas_filename) # ignore running mean files
                            ...
                            # if needed, repeat last year until entire period of interest is covered
                            if da_AFA.time.max().values.astype('datetime64[Y]').astype(int) < year_end:
                            ...
                            if flag_pic: 
                                ...
                                if '{}_{}'.format(d_isimip_meta[i]['model'],d_isimip_meta[i]['gcm']) not in pic_list:
                                    ...
                                    if  isinstance(file_names_pic, str): # single pic file 
                                        da_AFA_pic  = open_dataarray_isimi(file_names_pic)
                            ...
                            # save AFA field as pickle
                            with open('./data/pickles/isimip_AFA_{}_{}_{}.pkl'.format(extreme,mode,str(i)), 'wb') as f: # added extreme to string of pickle
                                pk.dump(da_AFA,f)
                    ...
                    for model in models: 
                        ...
                        for file_name in file_names: 
                            ...
                            da_AFA_rcp = open_dataarray_isimip(file_name, 'nmonths')

                            # save metadata
                            d_isimip_meta[i] = {
                                'model': file_name.split('_')[0].split('/')[-1], 
                                'gcm': file_name.split('_')[1], 
                                'rcp': file_name.split('_')[3],                         
                                'extreme': file_name.split('_')[6], 
                                'mode': mode,
                            }
                            ...
                            da_AFA_his = open_dataarray_isimip(file_name_his, 'nmonths')    
                    ...
                    for model in models: 
                        ...
                        for file_name in file_names: 
                            ...
                            da_AFA_rcp = open_dataarray_isimip(file_name, 'nevents')
                            ...
                            da_AFA_his = open_dataarray_isimip(file_name_his, 'nevents')    
                    ...
                    for model in models: 
                        ...
                        for file_name in file_names: 
                            ...
                            # load rcp data (AFA: Area Fraction Affected) - and manually add correct years
                            da_nevents_rcp = open_dataarray_isimip(file_name,'nevents')
                            da_nmonths_rcp = open_dataarray_isimip(file_name, 'nmonths')
                            ...
                            da_nevents_his = open_dataarray_isimip(file_name_his,'nevents')
                            da_nmonths_his = open_dataarray_isimip(file_name_his, 'nmonths')                  
                            ...
                            # concatenate historical and future data
                            da_nevents = xr.concat([da_nevents_his,da_nevents_rcp], dim='time')#.drop('year')
                            da_nmonths = xr.concat([da_nmonths_his,da_nmonths_rcp], dim='time')#.drop('year')
                            df_GMT = pd.concat([GMT_his,GMT_fut])
                            ...
                            # if needed, repeat last year until entire period of interest is covered
                            if da_nevents.time.max().values.astype('datetime64[Y]').astype(int) < year_end:
                                # line below was fixed; supposed to be average of last 10 years, but we only selected last year
                                # da_AFA_lastyear = da_AFA.sel(time=da_AFA.time.max()).expand_dims(dim='time',axis=0) # repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
                                da_nevents_lastyear = da_nevents.sel(time=slice(da_nevents.time.max()-9,da_nevents.time.max())).mean(dim='time').expand_dims(dim='time',axis=0)
                                da_nmonths_lastyear = da_nmonths.sel(time=slice(da_nmonths.time.max()-9,da_nmonths.time.max())).mean(dim='time').expand_dims(dim='time',axis=0)
                                # also adapted line below for GMTs
                                # GMT_lastyear = df_GMT.iloc[-1:,:]
                                GMT_lastyear = df_GMT.iloc[-10:,:].mean()

                                for year in range(da_nevents.time.max().values+1,year_end+1): 

                                    da_nevents = xr.concat([da_nevents,da_nevents_lastyear.assign_coords(time = [year])], dim='time')
                                    da_nmonths = xr.concat([da_nmonths,da_nmonths_lastyear.assign_coords(time = [year])], dim='time')
                                    if len(df_GMT) < 439: # necessary to avoid this filling from 2100-2113 if GMTs already go to 2299
                                        df_GMT = pd.concat([df_GMT,pd.DataFrame(data={'tas':GMT_lastyear['tas']},index=[year])])
                                    # changed below to above line so that new mean of last 10 years, like with afa, is properly appended to df_GMT
                                    # df_GMT = pd.concat([df_GMT,pd.DataFrame(GMT_lastyear).rename(index={0:year})])
                            ...
                            # retain only period of interest
                            da_nevents = da_nevents.sel(time=slice(year_start,year_end))
                            da_nmonths = da_nmonths.sel(time=slice(year_start,year_end))
                            df_GMT = df_GMT.loc[year_start:year_end,:]


                            d_isimip_duration = {'nevents' : da_nevents, 
                                                 'nmonths' : da_nmonths}
                ...
                with open('./data/pickles/isimip_metadata_{}_{}.pkl'.format(extreme,mode), 'wb') as f:
                    pk.dump(d_isimip_meta,f)
                with open('./data/pickles/isimip_pic_metadata_{}_{}.pkl'.format(extreme,mode), 'wb') as f:
        ...
        with open('./data/pickles/isimip_metadata_{}_{}.pkl'.format(extreme,mode), 'rb') as f:
            d_isimip_meta = pd.read_pickle(f)
        with open('./data/pickles/isimip_pic_metadata_{}_{}.pkl'.format(extreme,mode), 'rb') as f:
            d_pic_meta = pd.read_pickle(f)                    
...
def open_dataarray_isimip(file_name, da_name=False): 
    
    begin_year = int(file_name.split('_')[-2])
    end_year = int(file_name.split('_')[-1].split('.')[0])
    
    
    ds = xr.open_dataset(file_name, decode_times=False, engine='h5netcdf')

    # data variable name is not given 
    if not da_name: 
    ...
    # if not monthly then read annual files
